    ...
    return(<>
        ...
        <div className="container d-flex justify-content-center">
            {searchMovieListObj.slice(0, 5).map((singleMovie) => (
                <div className="card" key={singleMovie.original_title} style={{ width: '18rem', margin: '10px'}}>
                <img src={singleMovie.poster_path?'https://image.tmdb.org/t/p/original'+singleMovie.poster_path:'https://upload.wikimedia.org/wikipedia/commons/6/64/Poster_not_available.jpg'} className="card-img-top" alt="..."/>
                <div className="card-body">
                <h5>{singleMovie.original_title}{' ['+singleMovie.release_date.substring(0, 4)+']'}</h5>
                <p className="card-text">{singleMovie.overview.substring(0, 50)}</p>
                </div>
            </div>
            ))}
        </div>
